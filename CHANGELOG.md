...
### Changed
- **Renamed `TODO.md` → `ROADMAP.md`** to reflect that it's a forward-looking roadmap, not a done-log: completed items are removed after merge (unless they hang together with pending work), and `CHANGELOG.md` is the durable record of what shipped. All references updated (CLAUDE.md, AGENTS.md, Copilot instructions). Note: a naming overlap with `docs/dev/roadmap.md` remains, tracked as a roadmap item to resolve alongside defining the two files' roles.
