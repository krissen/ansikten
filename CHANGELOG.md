...
### Added
- **CLI subcommands (`ansikten culling` / `ansikten faces`)**: the terminal launcher now selects a workflow via a verb instead of always feeding files into the face queue. `ansikten culling DIR` (alias `cull`) opens/focuses the Gallra spelare module and loads the folder; `ansikten faces *.NEF` (or bare `ansikten *.NEF`) queues for face review as before. `--clear`/`-c` resets the target's working set before adding (alone = just empty); without it, paths are appended to the existing set. When the app is already running the args are routed into the live window (the right module opens/focuses). The launcher is now a versioned script in the repo (`bin/ansikten`) replacing the previous zsh function; all argument parsing lives in `src/main/cli-args.js` (unit-tested).
