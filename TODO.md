...
- [ ] **Retention på app-papperskorgen (Gallra)** — auto-rensa gallrade filer äldre än N dagar (default 30), användarinställbart i Inställningar. Gäller hela den app-hanterade papperskorgen (`TRASH_DIR`), dvs både NEF och JPEG. Manifestet (`manifest.jsonl`) har redan `trashed_at`, så purge = ta bort poster + lagrade filer äldre än tröskeln. Förslag: kör purge lazyt (vid backend-start och när papperskorgen öppnas) snarare än egen schemaläggare; lägg tröskeln i `config.json` + en Preferences-kontroll. Egen PR. **Rekommendation: ja** — håller papperskorgen från att växa obegränsat, med säker default och möjlighet att stänga av (0 = behåll för alltid).
